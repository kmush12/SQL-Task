...
Create output table.
```sql
SELECT msisdn, email_adress, CASE   WHEN gender = 'male' THEN 'M' WHEN gender = 'female' THEN 'F' ELSE 'U' END AS age_band,(DATE_PART('day', join_date) - 1) AS j_day,(DATE_PART('month', join_date)) AS j_month,(DATE_PART('year', join_date)) AS j_year,postal_sector,
    	handset_model,
    	handset_manufacturer,
    	needs_segment_name,
    	smart_phone_ind,
